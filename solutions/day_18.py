...
def find_surface_area(cubes: Iterable[Cube]) -> int:
...
def part_one(lava_cubes: list[Cube]) -> int:
    return find_surface_area(lava_cubes)
...
def all_cubes_in_space(boundries: Boundries) -> Iterator[Cube]:
...
def all_touching_pairs(boundries: Boundries) -> Iterator[frozenset[Cube, Cube]]:
    ...
    for cube in all_cubes_in_space(boundries=boundries):
        x, y, z = cube
...
def classify_cubes(lava_cubes: list[Cube]) -> dict:
...
def part_two(lava_cubes: list[Cube]) -> int:
    groups = classify_cubes(lava_cubes=lava_cubes)
    trapped_air = groups["trapped_air"]

    return find_surface_area(lava_cubes) - find_surface_area(trapped_air)
